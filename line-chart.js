...
class LineChart extends React.Component {
        ...
        {seriesIndex === this.state.sortedData.length - 1 && (
          <TouchableWithoutFeedback onPress={() => {
            ...
            this.state.sortedData.map((series) => {
              if (series.data[selectedIndex]) { //check undefined
  ...
  drawSelected(index) {
    if (this.state.sortedData.length === 0) return
    //get index with max data
    let maxCount = 0;
    let maxIndex = 0;
    this.state.sortedData.forEach((e, i) => {
      if (e.data) if (e.data.length > maxCount) {
        maxCount = e.data.length;
        maxIndex = i
      };
    });
    // got maxIndex
    let data = this.state.sortedData[maxIndex].data;//change from 0 to maxIndex
        ...
        <View style={StyleSheet.flatten([styles.selectedWrapper, {
          ...
          <View style={StyleSheet.flatten([styles.selectedBox])}>
            {this.state.sortedData.map((series) => {
              ...
              if (dataObject) { //check undefined or null
                return (
